...
function placer(G, options) {
  ...
  { ids.forEach(id => { const isS = !!listes.get(id).S; estS.set(id, isS);
    ...
    const resoudre = (colBlocs, guide) => {
      ...
      const aretes = f => { const E = [];
        ...
        colBlocs.map(c => c.filter(id => !estPastille(id))).forEach(c => { for (let i = 1; i < c.length; i++) { const P = c[i - 1], N = c[i];
          ...
          derniersDe(P).forEach(a => premiersDe(N).forEach(b => E.push([f(a), f(b), w]))); } });
        traversees.forEach(t => { if (t.dessus) derniersDe(t.dessus).forEach(a => E.push([f(a), f(t.net), mBas.get(t.dessus) + 6]));
          if (t.dessous) premiersDe(t.dessous).forEach(b => E.push([f(t.net), f(b), mHaut.get(t.dessous) + 6])); });
        return E; };
    ...
    for (let rp = 0; rp < 3; rp++) { const py2 = best.py; const sauve = new Map(); let bouge = false;
      ids.forEach(id => clesListes(id).forEach(lid => { const L = liste(id, lid); if (L.length < 2) return;
        ...
        grp.forEach(p => (partenaires.get(id + SEP + p.cle) || []).forEach(q => { if (q.id === id || dedoublee(q.id, q.cle)) return;
  ...
  { const dansCorps = (id, y) => { const isS = !!listes.get(id).S;
    ...
    const espaceOK = (id, cle, y) => { let ok = true;
      chaqueBroche(id, (p, lid) => { if (ok && p.cle !== cle && (listes.get(id).S || dedoublee(id, cle) || lid === listeDeBroche(id, cle)) && Math.abs(yB(id, p.cle) - y) < PRH - 0.5) ok = false; });
      return ok; };
...
function jugeDePlacement() {
  ...
  const juger = L => { if (!L.jugement) { const e = emprise(L), r = e.w / e.h; L.jugement = {
      sain: filsDansBloc(L) === 0, tient: r <= 2.2 && r >= 0.45, droits: compterDroits(L.routage.fils),
      croisements: compterCroisements(L.routage.fils, L.routage.barrettes), ec: r > A3 ? r / A3 : A3 / r,
      peignes: L.routage.barrettes.reduce((t, b) => t + Math.abs(b.y2 - b.y1), 0), largeur: e.w }; } return L.jugement; };
  const bat = (L, ref) => { if (!ref) return true; const a = juger(L), b = juger(ref);
    ...
    if (Math.abs(a.ec - b.ec) > 0.01) return a.ec < b.ec; return a.peignes < b.peignes; };
...
function meilleurPlacement(liaisons) {
  ...
  { const fix = goulottesOccupees(best);
    if (fix) { const L2 = essayer({ ...bOpt, goulottes: fix }); const a = juger(L2), b = juger(best);
      if (a.sain && a.droits >= b.droits && a.croisements <= b.croisements && a.largeur < b.largeur * 0.97) { best = L2; bGoulottes = fix; } } }
  ...
  { const L3 = essayer({ ...bOpt, goulottes: bGoulottes, condenser: true }); const a = juger(L3), b = juger(best);
    if (a.sain && a.droits >= b.droits && a.croisements <= b.croisements) best = L3; }
  // secours : serpentin
